...
if [[ $(curl -s -u ${GITHUB_ACTOR}:$GLOBAL_GITHUB_TOKEN https://api.github.com/repos/MultiSafepay/magento2${REPO_SUFFIX}/branches/${CURRENT_HEAD_REF} | grep -iGc '"name": "'${CURRENT_HEAD_REF}'"') == 1 ]]; then
...
if [[ $(curl -s -u ${GITHUB_ACTOR}:$GLOBAL_GITHUB_TOKEN https://api.github.com/repos/MultiSafepay/magento2${REPO_SUFFIX}-catalog-inventory/branches/${CURRENT_HEAD_REF} | grep -iGc '"name": "'${CURRENT_HEAD_REF}'"') == 1 ]]; then
    ADMIN_MODULE_BRANCH_NAME=${CURRENT_HEAD_REF}
else
    ADMIN_MODULE_BRANCH_NAME="master"
fi
...
# MAGENTO 2 MSI
if [[ $(curl -s -u ${GITHUB_ACTOR}:$GLOBAL_GITHUB_TOKEN https://api.github.com/repos/MultiSafepay/magento2${REPO_SUFFIX}-msi/branches/${CURRENT_HEAD_REF} | grep -iGc '"name": "'${CURRENT_HEAD_REF}'"') == 1 ]]; then
    ADMIN_MODULE_BRANCH_NAME=${CURRENT_HEAD_REF}
else
    ADMIN_MODULE_BRANCH_NAME="master"
fi
